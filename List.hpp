/*
  Filename   : List.hpp
  Author(s)  : William Killian, Gary M. Zoppetti, Trevor Bender
  Date       : 03-11-20
  Course     : CSCI 362-01
  Assignment : List
...
struct ListNode
{
  ...
  {
    // TODO
    ListNode* beginPrev = begin->prev;
    ListNode* endNext = end->next;

    beginPrev->next = endNext;
    endNext->prev = begin->prev;
  ...
  hook_range (ListNode* first, ListNode* last)
  {
    ...
    ListNode* next = this;
    // Set "first" prev equal to ListNode* "prev"
    first->prev = prev;
    // Set ListNode* "prev" next equal to "first"
    prev->next = first;
    // Set "last" next equal to ListNode* "next"
    last->next = next;
    //Set "next" prev equal to "last"
    next->prev = last;
...
public: /* should be private, but public for testing */
  ...
  transfer (iterator pos, iterator first, iterator last)
  {
    ...
    // Check if "first" and "last" are not equal
    if(first != last) {
      // Create new Node* pointing to "last" prev
      Node* lastNode = last.m_nodePtr->prev;
      // Unhook the range [first, last]
      Node::unhook_range(first.m_nodePtr, lastNode);
      // Hook range [first, last] before "pos"
      pos.m_nodePtr->hook_range(first.m_nodePtr, lastNode);
    }
...
public:
  ...
  // [5]
  explicit List (size_type count, const value_type& value = T ()) 
    : List ()
  {
    // TODO
    ...
    : List ()
  {
    ...
    // Check if distance between "first" and "last" iterator is 0
    if(std::distance(first, last) == 0){}
    else {
      // Loop through list until end
      while(first != last) {
        // Add value of "first" iterator to back of list
        push_back(*first);
        // Increment iterator "first"
        ++first;
      }
    }
  }
  ...
  insert (iterator pos, const value_type& value)
  {
    ...
    // Creates a new Node pointer with data equal to "value"
    Node* newNode = new Node(value);
    // Inserts "newNode" before "pos"
    pos.m_nodePtr->hook(newNode);
    // Increase "m_size" by 1
    ++m_size;
    // Return the iterator pointing to new element
    return --pos;
  }
  ...
  erase (iterator pos)
  {
    ...
    iterator nextNode = pos.m_nodePtr->next;
  ...
  reverse ()
  {
    ...
    while(node != end()) {
      std::swap(node.m_nodePtr->prev, node.m_nodePtr->next);
      --node;
    }
    std::swap(m_header.next, m_header.prev);
  }
